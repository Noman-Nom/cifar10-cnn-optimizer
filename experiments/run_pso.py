...
def _retrain_best(best_config, seed, max_epochs, es_patience, device):
    ...
    early_stopping = EarlyStopping(patience=es_patience, mode='max', restore_best_weights=True)
    history = trainer.train(train_loader=train_loader, val_loader=val_loader, max_epochs=max_epochs, early_stopping=early_stopping)
...
def _make_pso_objective(search_space, max_epochs, es_patience, device):
    def objective(config, trial_number=0):
        try:
            ...
            early_stopping = EarlyStopping(patience=es_patience, mode='max', restore_best_weights=True)
            trainer.train(train_loader=train_loader, val_loader=val_loader, max_epochs=max_epochs, early_stopping=early_stopping)
...
def main():
    ...
    for seed in seeds:
        ...
        pso = ParticleSwarmOptimizer(search_space=search_space, population_size=population_size, w=w, c1=c1, c2=c2, seed=seed)
        ...
        for iteration in range(n_iterations):
            ...
            for particle_idx in range(pso.population_size):
                ...
                pso.record_evaluation(config, score, metadata={'iteration': iteration + 1, 'particle': particle_idx + 1})
                opt_history.append({'iteration': iteration + 1, 'particle': particle_idx + 1, 'config': _to_python(config), 'score': float(score)})
            ...
            for particle_idx in range(pso.population_size):
                ...
                pso.velocities[particle_idx] = pso.w * pso.velocities[particle_idx] + cognitive + social
        ...
        test_acc, train_history = _retrain_best(best_config=pso.best_config, seed=seed, max_epochs=retrain_max_epochs, es_patience=retrain_es_patience, device=device)
